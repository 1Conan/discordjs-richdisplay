...
class ReactionHandler extends ReactionCollector {
    ...
    this.on('remove', (reaction, user) => {
      ...
      if (reaction.message.channel.type !== 'dm') {
		...
		this.on('end', () => {
      if (!this.reactionsDone  && this.message.deleted) return;
      if (this.message.channel.type !== 'dm') return this.message.reactions.removeAll();

      // Uses up a API requests so I disabled it
      // this.message.reactions.array().forEach((reaction) => {
